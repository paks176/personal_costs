...
export default {
    install(Vue) {
        Vue.prototype.$toast = {
            ...
            aria-live="assertive" 
            aria-atomic="true" 
            class="toast ease-animation overflow-hidden ${this.type}" 
            data-bs-autohide="${(this.type === 'info' || this.type === 'warning') ? 'true' : 'false'}"
            id="${this.toastId}">
            <div class="toast-wrapper opacity-0 d-flex rounded-8 p-3">
                ...
                if (window.toastContainer) {
                    window.toastContainer.insertAdjacentHTML('afterbegin', toastLayout);
            ...
            checkErrorCookie(toastId) {
                ...
                if (!messagesCookie) {
                    ...
                    if (!toasts) {
                        ...
                        const thisToast = toasts.split(',').find((toast) => toast === toastId);
                        if (!thisToast) {
                            this.setErrorCookie(toastId);
                            return true;
                        } else return false;
            ...
            setErrorCookie(toastId) {
                const errorsCookie = document
                    .cookie.split('; ')
                    .find((element) => element.startsWith('messages'));
                let resultString = errorsCookie ? '' : 'messages=';
                document.cookie = resultString + toastId;

            }

            showToast() {
                if (this.checkErrorCookie(this.toastId)) {
                    this.DOMElement.style.height = this.DOMElement.scrollHeight + 'px';
                    window.toastContainer.style.pointerEvents = 'auto';
                    this.DOMElement.classList.remove('opacity-0');
                }
            }

            hideToast(DOMElement, instance) {
                ...
                if (!window.toastContainer.querySelector('.toast')) {
                    window.toastContainer.style.pointerEvents = 'none';
                }
            }
        },
            processAll() {
